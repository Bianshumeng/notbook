...
public class Main {
    ...
    public static void saveState() {
        synchronized (notes) {
            ...
            try {
                ...
                //font family written last to keep backward compatibility
                for (Note n : notes) {
    ...
    /**
     * prefer a font that can render CJK; fall back to logical Dialog to avoid tofu
     */
    private static Font chooseUiFont(Font preferred, float size, boolean bold) {
        int style = bold ? Font.BOLD : Font.PLAIN;
        if (preferred != null) {
            Font sized = preferred.deriveFont(style, size);
            if (sized.canDisplay('汉') && sized.canDisplay('测')) {
                return sized;
            }
        }
