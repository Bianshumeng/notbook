...
public class Note extends JDialog {
    ...
    private final JMenuItem cut, copy, paste, delete, selectAll, fontSelect; //menu items inside copyPasteMenu
    ...
    private String textFontFamily = Main.BASE_FONT.getFamily(); //selected font family
    ...
    public Note() {
        ...
        text.setFont(getScaledFont(Main.TEXT_SIZE));
        ...
        fontSelect = new JMenuItem(getLocString("FONT"));
        fontSelect.setPreferredSize(new Dimension((int) (120 * Main.SCALE), (int) (36 * Main.SCALE)));
        fontSelect.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String[] fonts = GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames();
                Arrays.sort(fonts, String.CASE_INSENSITIVE_ORDER);
                JComboBox<String> chooser = new JComboBox<String>(fonts);
                chooser.setEditable(false);
                chooser.setSelectedItem(textFontFamily);
    ...
    public void setTextScale(float scale) {
        if (scale >= 0.99 && scale <= 1.01) {
            ...
            text.setFont(getScaledFont(Main.TEXT_SIZE * textScale));
        }
    }

    public void setTextFontFamily(String family) {
        setTextFontFamily(family, true);
    }

    public void setTextFontFamily(String family, boolean save) {
        if (family == null || family.trim().isEmpty()) {
            textFontFamily = Main.BASE_FONT.getFamily();
        } else {
    ...
    private Font getScaledFont(float size) {
        try {
            Font f = new Font(textFontFamily, Font.PLAIN, Math.round(size));
            if (!f.getFamily().equalsIgnoreCase(textFontFamily) && !textFontFamily.equalsIgnoreCase("Dialog")) {
                return Main.BASE_FONT.deriveFont(size);
            }
